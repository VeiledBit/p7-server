const LidlSaleItem = require("../models/LidlSaleItem");
...
const getItemsOnSaleSE = async (req, res) => {
  ...
  try {
    const index = meiliClient.index("lidlSaleItems");
...
const getItemsOnSale = async (req, res) => {
  ...
  try {
    const saleItems = await LidlSaleItem.findAll({
      where: whereClause,
      order: orderClause,
      limit: limit,
      offset: offset,
...
const getCategories = async (req, res) => {
  try {
    const distinctCategories = await sequelize.query(
      'SELECT DISTINCT "category_id", "category_name" FROM "lidlsales" ORDER BY "category_id";',
      {
